...
def synthesize_semantics(benchmark):
    ...
    cmd_prepare_base = ["sudo", "env", f"ELMFUZZ_RUNDIER=preset/{benchmark}", "python", os.path.join(PROJECT_ROOT, "prepare_fuzzbench.py")]
    match benchmark:
    ...
    subprocess.run(cmd_prepare_base, check=True, env=os.environ.copy(),
