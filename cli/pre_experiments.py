...
def synthesize_semantics(benchmark):
    ...
    subprocess.run(" ".join(cmd_prepare_base), check=True, env=os.environ.copy() | {"ELMFUZZ_RUNDIR": os.path.join(PROJECT_ROOT, "preset", benchmark)},
                   cwd=PROJECT_ROOT, stdout=sys.stdout, stderr=sys.stderr, user=USER, shell=True)
