...
```bash
docker pull ghcr.io/osuseclab/elfuzz:25.06.1
```
...
```bash
docker run --storage-opt size=150G --cpus 30 -it --add-host=host.docker.internal:host-gateway -v "/var/run/docker.sock:/var/run/docker.sock" --name elfuzz ghcr.io/osuseclab/elfuzz:25.06.1
```
...
- `ghcr.io/osuseclab/elfuzz:25.06.1`: Specifies the Docker image to use.
...
```bash
docker build -t ghcr.io/osuseclab/elfuzz:25.06.1 -f .devcontainer/Dockerfile --target publish .
```
